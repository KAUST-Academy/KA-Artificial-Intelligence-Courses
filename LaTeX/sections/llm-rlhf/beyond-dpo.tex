...
\begin{frame}[t,allowframebreaks]{Why an Odds Ratio}
  \begin{itemize}\itemsep0.4em
    ...
    \item[\textcolor{red}{$\blacktriangleright$}] The odds ratio supplies that missing downward pressure --- but gently. Hong et al.\ compare it against the plain probability ratio and find the latter ``leads to more extreme discrimination of the disfavored responses,'' suppressing token logits harder than a model still learning the task can absorb.
...
\begin{frame}[t]{KTO: Thumbs Up and Thumbs Down}
  \begin{itemize}
    ...
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{KTO} (Ethayarajh et al., 2024) learns from that unpaired signal using the value function from Kahneman and Tversky's prospect theory --- concave in gains, convex in losses, and measured against a reference point rather than in absolute terms:
  \end{itemize}
  \[
  ...
  \footnotesize
  $z_0$ is the model's own average KL from $\pi_{\text{ref}}$, estimated cheaply from mismatched pairs inside the microbatch. $\lambda_D$ and $\lambda_U$ are loss-aversion weights: keep $\lambda_D n_D / \lambda_U n_U$ in $[1, 3/2]$, so at a 1:10 desirable-to-undesirable ratio set $\lambda_U = 1$ and $\lambda_D \in [10,15]$. Note that $r_\theta$ still references $\pi_{\text{ref}}$ --- KTO drops the pairing, not the reference model.
\end{frame}
...
\begin{frame}[t]{Choosing a Preference Method}
  ...
  \vspace{0.4em}
  \begin{itemize}
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{The data you already have decides this, not the leaderboard.} Unpaired feedback points at KTO, which tolerates skew paired methods cannot --- discarding 90\% of the desirable data still beat DPO on Llama-7B. Curated pairs on a tight budget point at ORPO; pairs plus an existing SFT model point at DPO.
    \item[\textcolor{red}{$\blacktriangleright$}] PPO stays the most expressive of the four --- it optimises against a reward model, not a fixed dataset --- and the hardest to get right.
  \end{itemize}
\end{frame}
