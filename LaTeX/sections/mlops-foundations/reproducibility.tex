...
\begin{frame}{The Limits of Determinism}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{alertblock}{What frameworks actually promise}
                ...
                releases, individual commits, or different platforms'' --- nor, it adds, between CPU and
                GPU executions, even with identical seeds.
            \end{alertblock}
            \vspace{0.4em}
            \begin{block}{And what it costs}
                \small ``Deterministic operations are often slower than nondeterministic operations, so
                single-run performance may decrease for your model.''
            \end{block}
        \end{column}
    ...
    computer-vision course; here we stay framework-agnostic.}
