...
\begin{frame}{Proximal Policy Optimization (Recap)}
    \begin{itemize}\itemsep0.32em
      ...
      \item[\textcolor{red}{$\blacktriangleright$}] \textbf{Clipped objective} (widely used variant): with the probability ratio $\rho_t(\theta) = \frac{\pi_\theta(a_t \mid s_t)}{\pi_{\theta_{\mathrm{old}}}(a_t \mid s_t)}$,
        \[
          \mathcal{L}^{\mathrm{CLIP}} = \mathbb{E}_t\Big[\min\big(\rho_t(\theta)\hat{A}_t,\; \mathrm{clip}(\rho_t(\theta), 1-\epsilon, 1+\epsilon)\hat{A}_t\big)\Big]
        \]
