...
\begin{frame}{Classifier-Free Guidance (CFG)}
  \begin{itemize}\itemsep0.3em
    ...
    \item \textbf{Training trick:} replace the text conditioning with a null token for a random fraction of training \emph{examples} ($\sim$10\%), so one network learns both a \textbf{conditional} and an \textbf{unconditional} noise prediction.
...
\begin{frame}{Samplers: From 1000 Steps to 20}
  \begin{itemize}\itemsep0.28em
    ...
    \item \textbf{ODE-solver view:} sampling is integrating a differential equation, so better integrators reach good quality in fewer steps. Euler is first order but works well on the probability-flow ODE with a good noise schedule; DPM-Solver++ and Heun are higher order.
