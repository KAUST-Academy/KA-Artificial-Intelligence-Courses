...
% ================================================================ 3.2
% Deep RL from Human Preferences (Christiano 2017) - one clear goal
\begin{frame}{This Idea Was Proven Before LLMs (2017)}
\textbf{The point of this slide:} the recipe works, and works with surprisingly
little human input.
\vspace{0.3em}
\begin{columns}[c]
\begin{column}{0.52\textwidth}
\begin{itemize}\itemsep0.4em\small
    \item Christiano et al.\ gave the agent \emph{no} reward function at all.
    \item A human just watched \emph{pairs of short clips} and clicked which one
          looked better.
    \item From those clicks alone, labeling \empr{under 1\%} of the agent's
          experience, it learned to play Atari games and make a simulated robot
          do backflips.
\end{itemize}
\vspace{0.4em}
{\footnotesize \textbf{Takeaway:} learning a reward from \emph{comparisons} is
enough to train complex behaviour. RLHF scales this exact idea up to language.}
\end{column}
...
\end{column}
\end{columns}
\srcnote{\scriptsize Christiano, Leike, Brown, Martic, Legg, Amodei. \href{https://arxiv.org/abs/1706.03741}{Deep Reinforcement Learning from Human Preferences}. NeurIPS 2017.}
\end{frame}
...
\begin{itemize}\itemsep0.2em
    ...
    \item \textbf{Why the leash works:} the penalty subtracts reward that grows with
          KL distance, so each step away from $\pi_{\text{ref}}$ costs more. Past the
          peak, the extra proxy gain is outweighed by the penalty, so the
          best-scoring policy sits \emph{near the peak}, not in the hacked region.
\end{itemize}
\end{column}
\end{columns}
...
The \textbf{Step-1 objective} (maximize reward minus the $\beta$-KL leash) has a
known closed-form maximizer, a one-line result you can get with a single
Lagrange-multiplier calculation; we'll just use it:
\[
...
\]
\begin{itemize}\itemsep0.4em\small
    \item \textbf{What:} $\pi^\star$ is the policy that \emph{maximizes Step 1}.
          $Z(x)$ is a \emph{normalizer} (partition function) that makes the
          probabilities sum to 1; it depends \emph{only} on the prompt $x$.
          Remember that.
    \item \textbf{Why:} we don't have to run RL to know what the optimum looks like; the maths hands it to us directly.
    \item \textbf{How / reading the formula:} take the reference policy and reweight
          each response by $e^{\, r(y)/\beta}$. The exponential turns the (possibly
          negative) reward into a \emph{positive multiplier}, so higher-reward
          responses get up-weighted; $\beta$ sets how sharply. This $\pi_{\text{ref}}
          \cdot e^{\, r/\beta}$ shape (Gibbs/Boltzmann) is exactly what
          ``reward minus KL'' always produces.
\end{itemize}
\end{frame}
...
\end{align*}
\begin{itemize}\itemsep0.4em\small
    \item \textbf{What:} the reward now sits alone on the left, written entirely in
          terms of the policies.
    \item \textbf{Why:} we want the reward \emph{expressed through the policy}: if
          reward is just a function of $\pi^\star$, maybe we never need a separate
          reward model.
    \item \textbf{How:} the only snag is $\beta\log Z(x)$, and $Z(x)$ sums over
          \emph{all possible responses}, so it is impossible to compute. Step 4
          removes it.
\end{itemize}
\end{frame}
