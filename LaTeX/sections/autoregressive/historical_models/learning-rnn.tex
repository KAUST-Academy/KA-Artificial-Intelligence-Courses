...
\begin{figure}
    \centering
    \includegraphics[height=1.3in,keepaspectratio]{images/autoregressive/learning.png}
    \caption*{Learning a generative model. $d(\cdot)$ is a distance measure. \\
    \scriptsize Source: S. Ermon, Stanford CS236 Deep Generative Models, Lecture 3 ``Autoregressive Models'' (slides).}
\end{figure}
\end{frame}
...
\begin{frame}[allowframebreaks]{KL Divergence}
    \begin{itemize}
        ...
        \item $D(\mathcal{P}_{data}||\mathcal{P}_{\theta}) \geq 0$, with equality iff the two distributions are the same.
        \item It measures the ``compression loss'' (in bits) of using $\mathcal{P}_{\theta}$ instead of $\mathcal{P}_{data}$.
    \end{itemize}
\end{frame}

\begin{frame}[allowframebreaks]{Expected Log-Likelihood}
...
\begin{itemize}
    ...
    \begin{align*}
        D(\mathcal{P}_{data}||\mathcal{P}_{\theta}) & = E_{x \sim \mathcal{P}_{data}}   \left [log \left ( \frac{\mathcal{P}_{data}(x)}{\mathcal{P}_{\theta}(x)} \right ) \right ] \\
         & = \underbrace{E_{x \sim \mathcal{P}_{data}} [log \mathcal{P}_{data}(x)]}_{-H(\mathcal{P}_{data})} - E_{x \sim \mathcal{P}_{data}} [log \mathcal{P}_{\theta}(x)]
    \end{align*}
    where $H(\mathcal{P}_{data}) = -E_{x \sim \mathcal{P}_{data}}[log \mathcal{P}_{data}(x)]$ is the \textbf{entropy} of the data distribution: a property of the data alone, fixed once the dataset is given.
...
\begin{itemize}
    ...
    \item \textbf{Challenge}: model $\mathcal{P}(x_t|x_{1:t-1}; \alpha^t)$. The ``history'' $x_{1:t-1}$ keeps getting longer.
...
\begin{figure}
    ...
    \scriptsize Source: H. Dong, ``Sequential Models: Recurrent Neural Networks'', Deep Generative Models course, Peking University, 2022.}
\end{figure}
