...
app   = FastAPI()
MODEL = sio.load("model.skops", trusted=EXPECTED_TYPES)  # the whole fitted Pipeline
...
def predict(req: Request):
    X = pd.DataFrame([req.model_dump()])       # same column names as at training time
...
\begin{frame}{CPU or GPU? The Serving Economics}
    ...
    \begin{itemize}
        ...
        \item \textit{Going deeper:} model compression for efficient inference (quantisation, pruning,
              distillation) is covered in the NLP course's \textbf{Inference Optimisation} deck, and
              multi-GPU/multi-node serving in its \textbf{Distributed Training \& Serving} deck. We stay
