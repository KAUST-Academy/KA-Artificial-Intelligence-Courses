...
\begin{frame}[t]{Summary: Transformers 2017 vs.\ 2026}
  ...
  \begin{center}
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\linewidth}>{\raggedright\arraybackslash}p{0.37\linewidth}>{\raggedright\arraybackslash}p{0.37\linewidth}@{}}
      ...
      \hline
      Sequence mixer & Softmax attention is the only mixer & Attention, with SSM / linear-recurrence layers as the cheap mixer in some hybrid stacks \\
      \hline
...
\begin{frame}{Summary}
  \begin{itemize}
    ...
    \item New models (2024--2026) are designed together, optimizing attention, normalization, position encoding, sparsity (MoE), and serving (cache size, kernels).
    ...
    \item One thread stayed outside the block: state-space models (S4 $\to$ Mamba $\to$ Mamba-2) swapped attention for a fixed-size recurrent state, and by 2026 survive mainly as the cheap layer type inside hybrid stacks that keep a few attention layers for exact recall.
  \end{itemize}
\end{frame}
