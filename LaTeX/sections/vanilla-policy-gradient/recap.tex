...
\begin{frame}[allowframebreaks]{Recap}
    \begin{itemize}
        ...
        \item SARSA: On-policy version of Q-learning (learns from actions the policy actually takes).
        \item Deterministic vs.\ Stochastic policy: $\pi(s) = a$ vs.\ $\pi(a|s) = \Pr(a_t = a \mid s_t = s)$.
        \item Advantage function: $A^{\pi}(s,a) = Q^{\pi}(s,a) - V^{\pi}(s)$ — how much better an action is than average in that state.
    \end{itemize}
\end{frame}