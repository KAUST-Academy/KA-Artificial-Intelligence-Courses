...
\begin{frame}[allowframebreaks]{Autoregressive Models: Modern Models}
    ...
    \begin{itemize}
        ...
        \item Because every position is visible by default, an autoregressive Transformer needs a \textbf{causal mask} so that position $i$ can only attend to positions $< i$.
        \item Training then parallelizes over all positions at once, unlike an RNN. Generation is still one token at a time.
        \item The GPT family is the standard autoregressive example. Masked bidirectional encoders such as BERT are \emph{not} autoregressive: they predict masked tokens from both sides, so they cannot be sampled from left to right.
    ...
    \end{itemize}
\end{frame}
\input{sections/autoregressive/modern_models/pixelcnn}
\input{sections/autoregressive/modern_models/wavenet}