...
\begin{frame}{RAG Architecture}
    ...
    \begin{itemize}
        ...
        \item \textbf{Retriever:} Encodes the query and the documents into vectors and fetches the closest passages from the corpus by similarity search (e.g., DPR, ColBERT).
        \item \textbf{Generator} (also called the \textbf{reader})\textbf{:} An LLM that produces the final answer conditioned on the retrieved passages (e.g., BART, GPT).
    \end{itemize}
    \vspace{0.3em}
    \begin{center}
        \includegraphics[width=0.95\textwidth]{images/prompting-rag/rag_lewis2020_architecture.png}
    \end{center}
    \vspace{0.1em}
    {\scriptsize
        \color{gray!75!black}
