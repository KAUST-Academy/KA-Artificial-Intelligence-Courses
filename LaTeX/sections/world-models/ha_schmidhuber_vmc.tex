...
\begin{frame}{Vision module: variational autoencoder (V)}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\itemsep0.26em
      ...
      \item[\textcolor{red}{$\blacktriangleright$}] \textbf{VAE:} first stage of a two-stage sequence model; similar in spirit to modern tokenizers, but trained with the classic VAE objective (see the recap slide).
    \end{itemize}
...
\begin{frame}{Memory module: MDN-RNN (M)}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\itemsep0.26em
      ...
      \item[\textcolor{red}{$\blacktriangleright$}] \textbf{MDN-RNN:} an RNN (an LSTM, covered earlier) whose output parameterizes a \textbf{mixture of Gaussians} over the next latent $z_{t+1}$; a mixture can represent several plausible futures at once.
...
\begin{frame}{Training Inside the Dream}
  \begin{itemize}\itemsep0.3em
    ...
    \item This exploit-the-model problem returns in Dreamer, mitigated there by short imagination horizons and continual retraining on fresh real experience.
  \end{itemize}
\end{frame}
