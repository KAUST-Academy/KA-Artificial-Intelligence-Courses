...
\begin{frame}{Objective-Driven AI: Modules for Autonomous Intelligence}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\footnotesize\itemsep0.28em
    ...
    \centering
    \includegraphics[width=\linewidth]{images/jepa/turing_jepa_objective_driven_ami.jpg}\\[0.2em]
...
\begin{frame}{What Are Joint Embedding Predictive Architectures (JEPA)?}
  The model learns by predicting the \textbf{embedding} of one part of the input (e.g.\ a future segment or a masked region) from the representation of another (context / past), using paired encoders and a predictor.
...
% --- JEPA core world-model diagram (merged notation + walk-through) ---
\begin{frame}{JEPA: Architecture for the World Model}
  \begin{columns}[T,onlytextwidth]
    \column{0.38\textwidth}
    \begin{itemize}\itemsep0.3em
      \item \textbf{Notation:} $x$ = observed past/present; $y$ = future (or masked target); $a$ = action; $z$ = latent variable (what $x$ does not determine); $D()$ = prediction cost.
      \item Encoders map \textbf{$x$}, \textbf{$y$} to representations \textbf{$s_x$}, \textbf{$s_y$}.
      \item The \textbf{predictor} maps $(s_x, a, z)$ to \textbf{$\tilde{s}_y$}; training minimizes \textbf{$D(s_y,\tilde{s}_y)$} in representation space.
    \end{itemize}
    \column{0.62\textwidth}
    \centering
    \includegraphics[width=0.8\linewidth, height=0.8\textheight, keepaspectratio]{images/jepa/turing_jepa_core_predictor_diagram.png}
    \linebreak
    {\scriptsize Y.\ LeCun, ``Architecture for the world model: JEPA,'' slide 37, Ding-Shum Lecture, Harvard University, 2024 (as reproduced in Turing Post).}
  \end{columns}
\end{frame}
...
\begin{frame}{Multistep / Recurrent World Models}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\itemsep0.3em
      ...
      \item The same idea appears later in \textbf{V-JEPA 2-AC} for robot planning.
    \end{itemize}
    \column{0.58\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/jepa/turing_jepa_multistep_recurrent_wm.png}\\[0.2em]
    {\scriptsize ``Objective-Driven AI: Multistep/Recurrent World Model.'' Y.\ LeCun, slide 42, LMU M\"unchen, 2023 (as reproduced in Turing Post).}
  \end{columns}
\end{frame}
...
\begin{frame}{Hierarchical World Models and Planning}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\itemsep0.28em
      ...
      \item Hierarchical JEPA remains largely a \textbf{proposal} (LeCun, 2022); current systems implement only the single-level version.
    \end{itemize}
    \column{0.58\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/jepa/turing_jepa_hierarchical_planning.png}\\[0.2em]
    {\scriptsize ``Objective-Driven AI: Hierarchical Planning.'' Y.\ LeCun, slide 43, LMU M\"unchen, 2023 (as reproduced in Turing Post).}
  \end{columns}
\end{frame}
