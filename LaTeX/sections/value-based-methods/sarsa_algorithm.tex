\section{SARSA}
\begin{frame}{}
    \LARGE Reinforcement Learning: \textbf{SARSA}
\end{frame}

\begin{frame}{SARSA: Overview}
    \begin{itemize}
        \item \textbf{SARSA} stands for:
        \begin{align*}
            &\text{State} \\
            \rightarrow\ &\text{Action} \\
            \rightarrow\ &\text{Reward} \\
            \rightarrow\ &\text{State}' \\
            \rightarrow\ &\text{Action}'
        \end{align*}
        \item \textbf{On-policy learning:} Learns Q-values by following the current behavior policy.
        \item \textbf{Update Rule:}
        \begin{equation*}
            Q(s, a) \leftarrow Q(s, a) + \alpha \left[ r + \gamma Q(s', a') - Q(s, a) \right]
        \end{equation*}
    \end{itemize}
\end{frame}

\begin{frame}{SARSA: Key Points}
    \begin{itemize}
        \item Uses the action the agent actually takes, not just the best possible one.
        \item Learns from the agent's real experience, not hypothetical alternatives.
        \item Tends to be safer—useful in situations where mistakes are costly.
        \item Helps the agent improve its actual behavior step by step.
    \end{itemize}
\end{frame}

\begin{frame}{SARSA vs. Q-Learning}
    \begin{itemize}
        \item The SARSA algorithm is a slight variation of the Q-Learning algorithm.
        \item Q-Learning is \textbf{off-policy}: the Bellman update uses $\max_{a'} Q(s',a')$, which asks ``what is the \emph{best possible} action in $s'$?''---regardless of what the agent actually does.
        \item SARSA is \textbf{on-policy}: the update uses $Q(s',a')$ where $a'$ is the action the current policy actually selects, so it learns from real experience, not hypothetical alternatives.
    \end{itemize}
    \pause
    \begin{equation*}
        \textbf{Q-Learning:}\quad Q(s,a) \leftarrow Q(s,a) + \alpha \left[ r + \gamma \max_{a'} Q(s',a') - Q(s,a) \right]
    \end{equation*}
    \pause
    \begin{equation*}
        \textbf{SARSA:}\quad Q(s,a) \leftarrow Q(s,a) + \alpha \left[ r + \gamma Q(s',a') - Q(s,a) \right]
    \end{equation*}
\end{frame}

\begin{frame}{SARSA}
    \begin{itemize}
        \item The update equation for SARSA depends on the current state, current action, reward obtained, next state, and next action.
        \item This observation led to the naming of the learning technique as \textbf{SARSA}, which stands for \textbf{State-Action-Reward-State-Action}, symbolizing the tuple $(s, a, r, s', a')$.
        \pause
        \item Similar to DQN, there is also a Deep SARSA variant.
    \end{itemize}
\end{frame}

\begin{frame}{SARSA Algorithm}
    \begin{center}
    \footnotesize
    \begin{algorithm}[H]
        \DontPrintSemicolon
        \caption{SARSA (on-policy TD control)}
        Initialize $Q(s, a)$ arbitrarily for all $s, a$; set step size $\alpha \in (0, 1]$ and discount $\gamma$\;
        \For{each episode}{
            Observe initial state $s$\;
            Choose $a$ from $s$ using a policy derived from $Q$ (e.g.\ $\varepsilon$-greedy)\;
            \Repeat{$s$ is terminal}{
                Take action $a$; observe reward $r$ and next state $s'$\;
                Choose $a'$ from $s'$ using a policy derived from $Q$ (e.g.\ $\varepsilon$-greedy)\;
                $Q(s, a) \leftarrow Q(s, a) + \alpha \big[\, r + \gamma Q(s', a') - Q(s, a) \,\big]$\;
                $s \leftarrow s'$;\quad $a \leftarrow a'$\;
            }
        }
    \end{algorithm}
    \end{center}
    \footnotetext{Sutton \& Barto, \emph{Reinforcement Learning: An Introduction} (2018), Ch.\ 6. The next action $a'$ is chosen \emph{before} the update---this is what makes SARSA on-policy.}
\end{frame}

\begin{frame}{Q-Learning vs SARSA: Comparison Table}
    \begin{table}[h!]
        \centering
        \renewcommand{\arraystretch}{1.8}
        \begin{tabular}{lcc}
            \hline
            \textbf{Feature} & \textbf{Q-Learning} & \textbf{SARSA} \\
            \hline
            Policy Type & Off-policy & On-policy \\
            Learns From & Others' or past experience & Own current experience \\
            Target & $\max_{a'} Q(s', a')$ & $Q(s', a')$ actually taken \\
            Exploration Aware & No & Yes \\
            Risk Sensitivity & More aggressive & More conservative \\
            Use Case & Goal-seeking behavior & Risk-aware behavior \\
            \hline
        \end{tabular}
    \end{table}
    \vspace{0.3em}
    \small The on-policy vs.\ off-policy distinction is one of the most important axes in RL algorithm design and will appear in every remaining day of this course.
\end{frame}