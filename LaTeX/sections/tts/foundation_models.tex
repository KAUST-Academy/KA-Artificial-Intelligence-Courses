...
\begin{frame}{VALL-E: zero-shot TTS}
  \begin{itemize}
    ...
    \item The system takes as input a text prompt and a sample of the target voice, tokenizes both (using phonemes (grapheme-to-phoneme) for text and an audio codec for the speech), and uses a language model to generate discrete audio tokens for the text, matching the target speaker.
    \item VALL-E can do zero-shot TTS, speech editing, and content creation with other generative AI models like GPT-3.
  \end{itemize}
  \centering
  \includegraphics[width=0.92\textwidth,height=0.46\textheight,keepaspectratio]{images/tts/valle_overview_fig1.png}
...
\begin{frame}{VALL-E: comparison with other cascaded TTS systems}
  \centering
  \includegraphics[width=0.92\textwidth,height=0.44\textheight,keepaspectratio]{images/tts/valle_page2.png}
  \slideimgsource{Wang et al., VALL-E, Table~1: comparison with other cascaded TTS systems.}
\end{frame}
...
\begin{frame}{Voicebox: limitations}
  \begin{itemize}\footnotesize
    ...
    \item \textbf{Misuse:} high-quality cloning raises deepfake concerns; the authors release a strong synthetic-speech \textbf{classifier} as a mitigation.
  \end{itemize}
\end{frame}
