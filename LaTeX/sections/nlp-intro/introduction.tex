...
\begin{frame}{Text Preprocessing}
    \begin{itemize}
        ...
        \item \textbf{Language detection} (optional): \\
        {\small Identify each document's language before applying language-specific steps or routing it to the right model.}
        \item \textbf{Spellcheck / spelling correction} (optional): \\
        {\small Normalize typos and misspellings so variants map to the same token.}
