...
\begin{itemize}
    \item The optimal Bellman equation tells us what $Q^{\star}$ must satisfy:
    $$Q^{\star}(s,a) = r(s,a) + \gamma \max_{a'} Q^{\star}(s',a')$$
    \item We do not know $Q^{\star}$ --- but we can define a target for any current estimate $Q$:
    $$\text{Target} = r(s,a) + \gamma \max_{a'} Q(s',a')$$
    \item If $Q = Q^{\star}$ then Target $= Q(s,a)$ --- there is no error
    \item If $Q \neq Q^{\star}$ then (Target $- Q(s,a)$) is the \textbf{Bellman error} --- the gap between what our estimate predicts and what the Bellman equation says it should be
\end{itemize}
\end{frame}
...
\begin{frame}{$\varepsilon$-Greedy Policy}
\begin{itemize}
    \item Simple solution: $\varepsilon$-greedy policy
    \begin{itemize}
        \item With probability $1 - \varepsilon$, choose the optimal action according to Q
        \item With probability $\varepsilon$, choose a random action
    \end{itemize}
    \item $\varepsilon$-greedy policy still widely used today regardless of simplicity
    \item Key practical consideration: $\varepsilon$ is often annealed over training --- starting high (lots of exploration early on) and decaying toward a small value (mostly exploitation once $Q$ is well-estimated). We will revisit exploration more rigorously on Day 7.
\end{itemize}
\end{frame}
...
\begin{frame}{Convergence Properties}
    \begin{itemize}
        \item Q-learning converges to the optimal action-value function $Q^{\star}$ under certain conditions:
        \begin{itemize}
        ...
        \item Under these conditions, Q-learning is guaranteed to converge to $Q^{\star}$ with probability 1.
    \end{itemize}
\end{frame}
