...
\begin{itemize}
    ...
    \begin{itemize}
        \item Bandits formalize explore vs.\ exploit; UCB and Thompson give principled, $\mathcal{O}(\log T)$-regret algorithms
        \item Scaled to deep RL: bootstrapped DQN $\equiv$ posterior sampling, pseudo-counts $\equiv$ UCB in pixel space, curiosity / RND / Go-Explore are genuinely new
        \item The same dilemma is live in LLM RL today --- entropy collapse and pass@k (Day 9)
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Inverse RL} learns it from demonstrations --- MaxEnt IRL (2008) $\to$ GAIL / AIRL (2016--18) $\to$ today's reward models
        \item \textbf{Preference-based learning} learns it from comparisons (Christiano 2017)
        \item Read the other way, \emph{training a reward model is an IRL problem} --- the \emph{direct ancestor of RLHF} on Day 9
    \end{itemize}
...
\hline
One-state bandit                          & $\epsilon$-greedy, UCB1, Thompson sampling \\
...
Reward unwritable; comparisons            & Bradley--Terry preference learning $\to$ RLHF (Day 9) \\
\hline
\end{tabular}
\end{center}
\end{frame}
