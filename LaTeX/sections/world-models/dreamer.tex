...
\begin{frame}[t]{Dreamer: Learning Behaviors in Imagination}
  \begin{itemize}\itemsep0.28em
    ...
    \item Rollouts never touch pixels: start from latent states of real experience drawn from the replay buffer, then unroll the RSSM \textbf{prior} for a short horizon.
...
\begin{frame}[t]{DreamerV2: discrete world model \& V1 $\rightarrow$ V2}
  \begin{columns}
    ...
    \begin{itemize}\footnotesize
      ...
      \item \textbf{KL balancing} weights the prior and posterior gradient paths of the KL term differently (the prior is trained faster), preventing the posterior from collapsing onto an untrained prior.
    \end{itemize}
  \end{columns}
\end{frame} 
...
\begin{frame}[t]{DreamerV3: What Makes It Stable}
  ...
  \begin{itemize}\itemsep0.26em
    ...
    \item \textbf{Percentile return normalization:} returns are divided by an EMA of their 5th--95th percentile range, so exploration behaves consistently across dense and sparse rewards.
