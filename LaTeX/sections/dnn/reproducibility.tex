% Scope: seeding, determinism and checkpointing for a single training run.
% Experiment tracking, model registries and CI for models are taught under
% MLOps in the Introduction to AI course.
...
\begin{tikzpicture}[
    ...
    % style names must avoid reserved TikZ keys (out, in, at, to, step, scale)
...
\begin{frame}[allowframebreaks]{Seeding DataLoader Workers}
\begin{itemize}
...
\vspace{1em}
\begin{itemize}
    \item \texttt{use\_deterministic\_algorithms(True)} makes PyTorch pick a deterministic kernel wherever one exists; \texttt{cudnn.deterministic} does the same for convolutions alone.
...
\begin{tabular}{|l|l|}
    ...
    A long production run & No, use \texttt{benchmark=True} \\
    \hline
\end{tabular}
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \end{itemize}
    \item Aim for two things rather than bit-equality: a run you can repeat exactly on your own machine, and a \emph{distribution} of results, over several seeds, that you can quote elsewhere.
\end{itemize}
...
\begin{itemize}
    ...
    \item On ImageNet with a pretrained ResNet50 (50 seeds) the effect shrinks but does not vanish: $75.48 \pm 0.10$, with a min-to-max gap of $0.44$ points.
...
\begin{itemize}
    ...
    \item Containers, lockfile-driven CI, experiment trackers and model registries take this much further; they belong to MLOps, which the Introduction to AI course covers separately.
\end{itemize}
\end{frame}

\begin{frame}[allowframebreaks]{Checkpoint Discipline}
\begin{itemize}
    \item A file containing only \texttt{model.state\_dict()} is enough for inference and useless for resuming: the optimizer's momentum buffers and the learning-rate scheduler's step count are gone.
...
\end{itemize}
\footnotetext{Checkpoint pattern follows the PyTorch \href{https://docs.pytorch.org/tutorials/beginner/saving_loading_models.html}{Saving and Loading Models} tutorial}
\end{frame}
...
\vspace{1em}
\begin{itemize}
    \item Restore them with the matching setters (\texttt{torch.set\_rng\_state}, \texttt{torch.cuda.set\_rng\_state\_all}, \texttt{np.random.set\_state}, \texttt{random.setstate}) instead of calling \texttt{set\_seed} again.
    \item For most coursework this is overkill, but it is the reason a resumed run and an uninterrupted run rarely land on the same number.
\end{itemize}
\end{frame}
