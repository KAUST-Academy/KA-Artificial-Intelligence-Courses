...
\begin{frame}{REINFORCE}
\begin{itemize}
    \item Expected reward, where $r(\tau) := \sum_{t \geq 0} \gamma^t r_t$ is the discounted trajectory return:
        \begin{equation*}
        \begin{split}
...
\begin{frame}{REINFORCE Cycle}
    \begin{figure}
        \centering
        \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{images/vanilla-policy-gradient/reinforce_cycle.png}
    \end{figure}
    \footnotetext{Note: the diagram mentions TensorFlow/Keras; in this course we use PyTorch. The steps are framework-agnostic.}
\end{frame}