...
\begin{frame}{Neural Audio Codecs: Audio as Tokens}
  \begin{itemize}\footnotesize
    ...
    \item An encoder downsamples the waveform, \textbf{residual vector quantization} maps each frame to entries of learned codebooks (each stage quantizes what the previous stage got wrong, so more codebooks means finer detail), and a decoder reconstructs the waveform from the codes.
    \item The result: audio becomes a token sequence, the same kind of object a language model already consumes and produces.
  \end{itemize}
  \centering
  \includegraphics[width=0.85\textwidth,height=0.40\textheight,keepaspectratio]{images/tts/encodec_architecture_rvq_codec.png}
  \slideimgsource{EnCodec: encoder, residual vector quantizer, decoder, and discriminator. D\'efossez et al., ``High Fidelity Neural Audio Compression,'' Fig.~1; \href{https://arxiv.org/abs/2210.13438}{arXiv:2210.13438}.}
\end{frame}
