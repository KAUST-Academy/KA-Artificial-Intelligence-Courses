...
\begin{frame}[t]{Task Vectors and Task Arithmetic}
  \begin{itemize}\itemsep0.4em
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{Key idea:} What a fine-tune \emph{learned} is a direction in weight space. Subtract the starting point and you can keep it, move it, and add it to something else (Ilharco et al., 2023).
      ...
      \]
    \item[\textcolor{red}{$\blacktriangleright$}] This is element-wise arithmetic on checkpoints: no gradients, no data, no GPU. The coefficient $\lambda$ is the one real hyperparameter, and it is tuned on a held-out set, not derived.
    \item[\textcolor{red}{$\blacktriangleright$}] Three operations, all of which do something useful:
      \begin{itemize}
        \item \textbf{Negation} ($-\tau$) degrades the target behaviour while leaving control tasks roughly intact --- used to make a model less toxic.
...
\begin{frame}[t,allowframebreaks]{Merging Recipes}
  \begin{itemize}\itemsep0.35em
    ...
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{SLERP:} interpolate along the geodesic rather than the straight line, so the result keeps its norm instead of shrinking toward the origin.
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{TIES} (Yadav et al., 2023) attacks the two ways task vectors interfere --- redundant entries and sign disagreement --- in three steps:
      \begin{itemize}
        ...
        \item \textbf{Disjoint merge:} average \emph{only} the models that agree with the elected sign.
      \end{itemize}
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{DARE} (Yu et al., 2024) is a preprocessing step, not a merge: \textbf{d}rop delta parameters at rate $p$ \textbf{a}nd \textbf{re}scale the survivors by $1/(1-p)$, so the expected update is unchanged. Drop rates of 90\% --- sometimes 99\% --- leave quality intact, which tells you how redundant a fine-tune's delta really is.
...
\begin{frame}[t]{Merging LoRA Adapters, and When It Fails}
  \begin{itemize}\itemsep0.45em
    ...
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{The failure mode is silent.} A bad merge still loads, still generates fluent text, and still scores respectably on the benchmarks the parents were good at. Safety behaviour and instruction-following degrade first and are exactly what a quick eval will not catch --- so evaluate the merge, never the recipe.
  \end{itemize}
\end{frame}
...
\begin{frame}[t,allowframebreaks]{Continual Learning for Deployed Models}
  \begin{itemize}\itemsep0.4em
    ...
    \item[\textcolor{red}{$\blacktriangleright$}] \textbf{Constrain the update.} Biderman et al.\ (2024) found LoRA learns \emph{less} than full fine-tuning on code and maths --- full fine-tuning finds updates of far higher rank --- but forgets less, and preserves the base model's behaviour outside the target domain better than weight decay or dropout.
