...
\begin{itemize}
    ...
    \item<2-> Learn model parameters by maximizing the likelihood of the training data (maximum likelihood, as for the models seen so far).
    $$p_{\theta}(x) = \int p_{\theta}(z) p_{\theta}(x|z) dz$$
    \item<3-> But there is a problem here. It is intractable to compute $p(x|z)$ for every $z$!
...
\begin{frame}[allowframebreaks]{Variational Inference}
    ...
    \begin{itemize}
        \item Let's approximate $p(z|x)$ with another distribution $q(z)$
