...
\begin{frame}[t]{RSSM: Dynamics}
  \begin{itemize}
    \item \textbf{Deterministic recurrence:} the GRU carries memory and the \textbf{action} forward,
...
\begin{frame}[t]{RSSM: Training Objective}
  ...
  \begin{itemize}\itemsep0.25em
    ...
    \item The \textbf{KL term} keeps the prior (used during imagination) and the posterior (informed by real observations) close together, so imagined futures stay realistic; how to weight its two directions is revisited by DreamerV2's KL balancing.
  \end{itemize}
\end{frame}
...
\begin{frame}[t]{PlaNet: Planning in Latent Space}
  \begin{itemize}\itemsep0.28em
    ...
    \item This is the same CEM / model-predictive control (\textbf{MPC}) loop that V-JEPA 2-AC uses (covered earlier); PlaNet did it with an ELBO-trained generative model and a reward head instead of goal images.
