...
\textbf{Introduced by:} Frey (1998); extended with hidden layers by Bengio \& Bengio (2000)
...
\textbf{Key Features:}
\begin{itemize}
    \item No conditional independence is assumed: the chain rule factorization is exact, and each variable is conditioned on \emph{all} of its predecessors.
    \item What is restricted is the \emph{form} of each conditional, which is a single sigmoid unit applied to a linear function of the predecessors.
...
\begin{figure}
    ...
    \caption*{FVSBN results. Left: training data. Right: averaged synthesized samples. \\
    \scriptsize Source: Gan, Henao, Carlson \& Carin, ``Learning Deep Sigmoid Belief Networks with Data Augmentation'', AISTATS 2015, Fig.~4 (left and middle panels).}
\end{figure}
