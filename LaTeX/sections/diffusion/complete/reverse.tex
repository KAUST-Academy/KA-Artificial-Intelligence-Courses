...
\begin{frame}{Reverse Denoising Process}
    ...
    \begin{itemize}
        \item<2-> We start with a sample from the noise distribution $p(\mathbf{x}_T) = \mathcal{N}(\mathbf{x}_T; \mathbf{0}, \mathbf{I})$.
...
\framebreak
        ...
        \item If $\beta_t$ is small enough at each time step, the true reverse conditional is approximately Gaussian, so we can model $p_\theta(\mathbf{x}_{t-1}|\mathbf{x}_t)$ as:
        $$
...
\framebreak
    ...
    \begin{figure}
        \centering
        \includegraphics[height=0.30\textheight, width=\textwidth, keepaspectratio]{images/diffusion/diff_4.png}
