...
\begin{frame}[t]{Attention's cost, and the recurrent alternative}
  \begin{itemize}\itemsep0.45em
    ...
    \item The state-space line asks for both at once: a constant-size state \emph{and} training that is parallel across the sequence. (Shrinking or paging the cache is the other answer to the same problem, and belongs to the inference-optimization lecture.)
  \end{itemize}
\end{frame}

\begin{frame}[t]{S4: a structured linear recurrence}
  \begin{itemize}\itemsep0.15em
    \item Start from a continuous linear system $h'(t) = Ah(t) + Bu(t)$, $y(t) = Ch(t)$ and discretize it with a step size $\Delta$: $h_t = \bar{A}h_{t-1} + \bar{B}x_t$, $y_t = Ch_t$. A linear RNN, with no nonlinearity in the state update.
...
\begin{frame}[t]{The Mamba block and the parallel scan}
  ...
  {\scriptsize The block folds H3 and a gated MLP into one repeated unit. Gu \& Dao, \href{https://arxiv.org/abs/2312.00752v2}{arXiv:2312.00752v2}, Figure~3.}
\end{frame}
...
\begin{frame}[t]{Hybrids: Jamba, Griffin, and after}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\itemsep0.15em
      ...
      \item By 2025 hybrids are the shipping form: Zamba2, Nemotron-H, Falcon-H1, RecurrentGemma --- the sliding-window recipe with a cheaper layer type.
    \end{itemize}
...
\begin{frame}[t]{Verdict (2026)}
  \begin{itemize}\itemsep0.45em
    ...
    \item Which is this deck's recurring lesson once more: the changes that stuck are the ones that move less memory per token. Other non-Transformer directions, such as byte-level and diffusion language models, are taken up in the recent-advancements lecture.
  \end{itemize}
\end{frame}
