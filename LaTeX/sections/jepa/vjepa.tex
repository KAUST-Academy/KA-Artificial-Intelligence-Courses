...
\begin{frame}{V-JEPA: Masking in Space \emph{and} Time}
  \centering
  \includegraphics[width=0.8\linewidth, height=0.8\textheight, keepaspectratio]{images/jepa/vjepa_blog_spatiotemporal_figure.png}\\[0.25em]
  {\scriptsize Decorative Meta AI illustration (hero art of the \href{https://ai.meta.com/blog/muavic-audio-visual-speech-translation-benchmark/}{MuAViC blog post}, 2023); not a V-JEPA figure.}
  \begin{itemize}\footnotesize
    \item Masking must be hard enough: \textbf{sparse random patches} make the task too easy, and little is learned about dynamics.
    ...
    \item V-JEPA uses masking across \textbf{both space and time} so the model must infer what happens in the scene rather than copy texture from nearby frames.
  \end{itemize}
\end{frame}
...
\begin{frame}{V-JEPA: Model Architecture}
  ...
  \begin{itemize}
        ...
        \item \textbf{Target path:} an EMA encoder processes the full unmasked clip; targets are its output tokens at the masked positions (stop-gradient). Loss matches predicted and target embeddings.
        \item Same high-level recipe as I-JEPA, extended to \textbf{3D} patch tokens over space and time.
  \end{itemize}
    \column{0.56\textwidth}
      \centering
      \includegraphics[width=0.95\linewidth, height=0.9\textheight, keepaspectratio]{images/jepa/vjepa_encoder_training.png}\\[0.25em]
      {\scriptsize Reference: \href{https://ai.meta.com/blog/v-jepa-yann-lecun-ai-model-video-joint-embedding-predictive-architecture/}{Meta V-JEPA blog}, 2024 (blog graphic).}
  \end{columns}
\end{frame}

% --- V-JEPA: Frozen evaluation (Meta blog) ---
\begin{frame}{V-JEPA: Frozen Evaluation Across Tasks}
  \begin{columns}[T,onlytextwidth]
  ...
  \end{itemize}
    \column{0.56\textwidth}
    \centering
    \includegraphics[width=0.95\linewidth, height=0.9\textheight, keepaspectratio]{images/jepa/vjepa_downstream_tasks.png}\\[0.25em]
    {\scriptsize Reusing frozen representations for downstream video understanding. Reference: \href{https://ai.meta.com/blog/v-jepa-yann-lecun-ai-model-video-joint-embedding-predictive-architecture/}{Meta V-JEPA blog}, 2024 (blog graphic).}
  \end{columns}
\end{frame}
...
\begin{frame}{V-JEPA: Results and Limitations}
  ...
  \end{itemize}
  \centering
  \includegraphics[width=0.8\linewidth, height=0.42\textheight, keepaspectratio]{images/jepa/vjepa_blog_eval_chart.png}\\[0.25em]
  {\scriptsize Low-shot frozen evaluation on Kinetics-400 and Something-Something-v2 (Meta V-JEPA blog; \href{https://arxiv.org/abs/2404.08471}{Bardes et al.}).}
\end{frame}
